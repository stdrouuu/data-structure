...
def insert(node):
    ...
    node = start.next
    last = start
    while node: #akan terus berjalan selama node tidak None.
        node = node.next
        last = last.next
...
def oldest_student():
    ...
    oldest = node  #Inisialisasi "oldest" dengan node pertama
    while node:  #Loop melalui semua node
        # Bandingkan tahun lahir
        if node.yob < oldest.yob:  #Jika tahun lahir < tahun lahir yang dianggap tertua,
            oldest = node  #Perbarui "oldest" ke node saat ini
        elif node.yob == oldest.yob: #jika tahun sama, maka masuk ke blok elif lalu ke blok if
            
        #Jika tahun sama, bandingkan bulan lahir
            if node.mob < oldest.mob:  #Jika bulan lahir < bulan lahir yang dianggap tertua,
                oldest = node  #Perbarui "oldest" ke node saat ini
            elif node.mob == oldest.mob:  #Jika bulan lahir sama, maka masuk ke blok elif lalu ke blok if
                
        #Jika bulan sama, bandingin hari lahir
                if node.dob < oldest.dob:  #Jika hari lahir < hari lahir yang dianggap tertua
                    oldest = node  # Perbarui "oldest" ke node saat ini
        node = node.next  # Pindah ke node berikutnya

    #Setelah menemukan siswa tertua, tampilkan informasinya
